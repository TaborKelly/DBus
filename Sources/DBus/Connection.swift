...
public final class DBusConnection {
    ...
    public init(address: String, shared: Bool = true) throws {
        ...
        if shared {
            ...
            self.internalPointer = dbus_connection_open(address, &error.cError)

        } else {

            self.internalPointer = dbus_connection_open_private(address, &error.cError)
        }
    ...
    public init(busType: DBusBusType, shared: Bool = true) throws {
        ...
        if shared {
            guard let p = dbus_bus_get(internalBusType, &error.cError) else {
                if error.isSet {
                    throw error
                } else {
                    throw RuntimeError.generic("dbus_bus_get() returned nil")
                }
            }
            self.internalPointer = p
        } else {

            self.internalPointer = dbus_bus_get_private(internalBusType, &error.cError)
        }
    ...
    public func send(message: DBusMessage) throws -> dbus_uint32_t {
        ...
        guard Bool(dbus_connection_send(internalPointer, message.internalPointer, &serialNumber))
            else { throw RuntimeError.generic("dbus_connection_send() failed") }
    ...
    public func sendWithReply(message: DBusMessage, timeout: Timeout = .default) throws -> DBusPendingCall? {
        ...
        guard Bool(dbus_connection_send_with_reply(internalPointer, message.internalPointer, pendingCallDoublePointer, timeout.rawValue))
            else { throw RuntimeError.generic("dbus_connection_send_with_reply() failed") }
